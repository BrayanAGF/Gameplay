...
export const SidebarComponent = () => {
    ...
    return (
        <div className={`${!MenuExpandido ? 'hidden sm:inline sm:w-[12%] md:w-[10%] lg:w-[8%] xl:w-[6%]' : "md:w-[18%] xl:w-[10%]"}  h-full p-1 flex flex-col gap-1 border-r-1 border-content1 transition-all Subtitulo`}>
            <Button isIconOnly={!MenuExpandido} className={`h-[50px] w-full flex bg-transparent hover:bg-overlay ${!MenuExpandido ? 'justify-center p-8' : 'justify-start'}`}>
                <div className={`flex ${!MenuExpandido ? 'flex-col gap-0' : 'gap-2'} items-center`}>
                    ...
                    <h5 className={`${!MenuExpandido ? "text-sm" : "text-lg" }`}>Inicio</h5>
                </div>
            </Button>
            <Button isIconOnly={!MenuExpandido} className={`h-[50px] w-full flex bg-transparent hover:bg-overlay ${!MenuExpandido ? 'justify-center p-8' : 'justify-start'}`}>
                <div className={`flex ${!MenuExpandido ? 'flex-col gap-0' : 'gap-2'} items-center`}>
                    ...
                    <h5 className={`${!MenuExpandido ? "text-sm" : "text-lg" }`}>Explorar</h5>
                </div>
            </Button>
            <Button isIconOnly={!MenuExpandido} className={`h-[50px] w-full flex bg-transparent hover:bg-overlay ${!MenuExpandido ? 'justify-center p-8' : 'justify-start'}`}>
                <div className={`flex ${!MenuExpandido ? 'flex-col gap-0' : 'gap-2'} items-center`}>
                    ...
                    <h5 className={`${!MenuExpandido ? "text-sm" : "text-lg" }`}>Biblioteca</h5>
                </div>
            </Button>
        </div>
    )
